...
def quaternion_from_euler(theta:float, phi:float, psi:float):
    cy = np.cos(psi * 0.5)
    sy = np.sin(psi * 0.5)
    cp = np.cos(phi * 0.5)
    sp = np.sin(phi * 0.5)
...
def euler_from_quaternion(q:list):
    x, y, z, w = q
    t0 = +2.0 * (w * x + y * z)
    t1 = +1.0 - 2.0 * (x * x + y * y)
...
def Array2ServoAnglesStamped(Theta, stamp):
    ...
    msg.header.stamp = stamp
    for i in range(6):
        msg.angles.append(np.rad2deg(Theta[i]))
    return msg

def ServoAnglesStamped2Array(msg:ServoAnglesStamped):
    Theta = np.deg2rad(np.asarray(msg.angles))
    return Theta
